...
section.addEventListener("click",async e=>{
 ...
 try{
  ...
  if(["add","addPage","remove","up","down","addOption","removeOption"].includes(action)){
   ...
   if(action==="up"&&i>1&&!isPage(saved.questions[i]))[saved.questions[i-1],saved.questions[i]]=[saved.questions[i],saved.questions[i-1]];
...
function normalizePages(){
 ...
 if(!out.length||!isPage(out[0]))out.unshift(pageHeader('첫 페이지',false));
 out[0].pageBreakBefore=false;saved.questions=out;
}
...
function enhanceEditor(){
 ...
 el.addEventListener('drop',e=>{e.preventDefault();if(!draggedQuestion)return;const from=saved.questions.findIndex(x=>x.id===draggedQuestion);draggedQuestion=null;if(from<0||from===i)return;const moving=saved.questions[from];saved.questions.splice(from,1);let to=saved.questions.findIndex(x=>x.id===q.id);if(isPage(q))to++;saved.questions.splice(Math.max(1,to),0,moving);activeQuestion=saved.questions.indexOf(moving);dirty=true;renderQuestions();section.querySelector('[data-index="'+activeQuestion+'"] summary')?.focus();});
 });updateLivePreview();
}
