...
function doPost(e) {
  ...
  try {
    ...
    if (body.file && body.file.base64 && body.file.name) {
      const fileName = `${safeName_(application.applicant_name)}_${application.id}_${safeName_(body.file.name)}`;
      const existingUpload=applicantFolder.getFilesByName(fileName);
      if (existingUpload.hasNext()) uploadedFileUrl=existingUpload.next().getUrl();
      else {
        ...
        uploadedFileUrl=applicantFolder.createFile(blob).getUrl();
      }
    }
...
function nurimSurveyRoute_(b) {
  ...
  if(b.action==="nurim-survey-health")return {ok:true,protocol:protocol,ready:true,capabilities:["application-extras-v87","web-consent-v90"]};
  ...
  if(b.action==="nurim-survey-sync-basic"){
    ...
    SpreadsheetApp.flush();nurimCsv_(folder,book);nurimCsv_(programFolder,book,r.program_title,app.program_id);
