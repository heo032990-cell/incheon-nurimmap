...
function nurimSurveyRoute_(b) {
  ...
  if(b.action==="nurim-survey-health")return {ok:true,protocol:protocol,ready:true,capabilities:["application-extras-v87"]};
  ...
  if(b.action==="nurim-survey-submit"){
    ...
    if(b.file){var uploadName=safeName_(r.name)+'_첨부_'+r.id+'_'+safeName_(b.file.name);var found=answersFolder.getFilesByName(uploadName);var upload=found.hasNext()?found.next():answersFolder.createFile(Utilities.newBlob(Utilities.base64Decode(b.file.base64),b.file.mimeType,uploadName));r.upload_url=upload.getUrl();}
    ...
    SpreadsheetApp.flush();
    nurimCsv_(folder,book);nurimCsv_(programFolder,book,r.program_title,b.program?b.program.id:b.application.program_id);
...
function nurimPdf_(folder,name,s,r){
 ...
 if(r&&r.extra){s=JSON.parse(JSON.stringify(s));r=JSON.parse(JSON.stringify(r));var x=r.extra;var details='참여자 구분: '+(x.participantType||'')+'\n요청사항: '+(x.note||'')+'\n서명: '+(x.signature||'');(x.consentItems||[]).forEach(function(c){var rows=c.type==='matrix'?(c.rows||[]):[{id:c.id,label:c.title}];details+='\n'+(c.text||'');rows.forEach(function(row){details+='\n'+(row.label||c.title)+': '+(x.consentResponses[row.id]==='agree'?'동의':'미동의');});});s.questions.push({id:'nurim_base_consent',type:'notice',label:'기본 신청 및 개인정보 동의',help:details,options:[],required:false});}
