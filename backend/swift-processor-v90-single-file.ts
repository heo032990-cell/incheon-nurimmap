// Supabase Edge Functions → swift-processor → index.ts 전체 교체용
// 기존 환경변수와 함수 설정을 유지합니다. SQL Editor에 넣는 코드가 아닙니다.
function googleBasicApplication(a: any){return Object.fromEntries(['id','program_id','applicant_name','phone','birth_date','created_at','application_status','queue_number','lifecycle_status','modified_at','cancelled_at','deleted_at','last_action_actor','last_action_at','drive_uploaded_file_url'].map(k=>[k,a[k]]));}
function googleProgram(p: any){return Object.fromEntries(['id','title','selection_method','capacity'].map(k=>[k,p[k]]));}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const targetFolderId = folderMatch?.[1] || "1X398D_XyevK0abng_g7XsVJ1FagGO_EF";
