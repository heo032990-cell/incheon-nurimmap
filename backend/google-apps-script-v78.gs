 ...
 * 대상 폴더: 테스트용
 * https://drive.google.com/drive/folders/1X398D_XyevK0abng_g7XsVJ1FagGO_EF
 */
const TARGET_FOLDER_ID = "1X398D_XyevK0abng_g7XsVJ1FagGO_EF";
...
function doPost(e) {
  ...
  try {
    ...
    const applicantFolder = getOrCreateApplicantFolder_(
      programFolder, applicantFolderName, String(application.id).slice(0, 8)
    );


    if (body.file && body.file.base64 && body.file.name) {
      const fileName = `${safeName_(application.applicant_name)}_${safeName_(body.file.name)}`;
      if (!applicantFolder.getFilesByName(fileName).hasNext()) {
...
function upsertProgramRosterCsv_(programFolder, application, program, applicantFolder) {
  ...
  const oldConsentHeaders = oldHeaders.filter(function(header) {
    return String(header || "").indexOf("동의 - ") === 0;
  });


  const consentEntries = Object.entries(application.consent_responses || {});
  const currentConsentHeaders = consentEntries.map(function(entry) {
    return "동의 - " + consentLabel_(program, entry[0]);
  });
  const consentHeaders = oldConsentHeaders.slice();
  currentConsentHeaders.forEach(function(header) {
    if (consentHeaders.indexOf(header) < 0) consentHeaders.push(header);
  });
  const headers = ROSTER_FIXED_HEADERS.concat(consentHeaders, [ROSTER_FOLDER_HEADER]);
  const rows = [headers];
...
function fetchPromotionPage_(value) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    response = UrlFetchApp.fetch(url, {
      ...
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; NurimProgramReader/1.1; +https://incheon-nurimmap.netlify.app/)",
...
function nurimSurveyRoute_(b) {
  ...
  if(b.action==="nurim-survey-health")return {ok:true,protocol:protocol,ready:true};
  if(!/^[a-f0-9-]{36}$/i.test(b.surveyId))throw new Error("설문 ID 오류");
  var folder=getOrCreateFolder_(root,"설문_"+b.surveyId);
  ...
  if(b.action==="nurim-survey-submit"){
    ...
    var programFolder=getOrCreateFolder_(folder,"프로그램_"+b.program.id);
    var answersFolder=getOrCreateFolder_(programFolder,"작성된신청서");
    var pdf=nurimPdf_(answersFolder,"신청서_"+r.id+".pdf",b.schema,r);
  ...
  if(b.action==="nurim-survey-sync-basic"){
    ...
    var row=idx+2,r=JSON.parse(tab.getRange(row,10).getValue());
    ...
    var programFolder=getOrCreateFolder_(folder,"프로그램_"+app.program_id);
    var answersFolder=getOrCreateFolder_(programFolder,"작성된신청서");
    var name="신청서_"+app.id+"_"+String(app.last_action_at).replace(/[^0-9]/g,"")+".pdf";
    ...
    SpreadsheetApp.flush();nurimCsv_(folder,book);
...
function nurimPdf_(folder,name,s,r){
 ...
 var doc=DocumentApp.create(name.replace(/\.pdf$/,"")),file=DriveApp.getFileById(doc.getId());
 file.moveTo(folder);
 try{
  var body=doc.getBody();body.setPageWidth(595).setPageHeight(842).setMarginTop(36).setMarginBottom(36).setMarginLeft(36).setMarginRight(36);
  body.setAttributes({FONT_FAMILY:"Malgun Gothic",FONT_SIZE:10});
  body.appendParagraph(s.title).setHeading(DocumentApp.ParagraphHeading.HEADING1);
  if(s.description)body.appendParagraph(s.description);
  var date=r?r.birth.replace(/(\d{4})-(\d{2})-(\d{2})/,"$1년 $2월 $3일"):"______년 ___월 ___일";
  body.appendTable([["이름: "+(r?r.name:"________________"),"생년월일: "+date],["연락처: "+(r?r.phone:"________________"),r?"접수: "+r.created_at:""]]);
  var pair=[];
  function flush(){if(pair.length){body.appendTable([pair.length===1?[pair[0],""]:pair]);pair=[];}}
  s.questions.forEach(function(q,i){
    var a=r?nurimAnswer_(q,r.answers[q.id]):q.type==="date"?"______년 ___월 ___일":q.type==="consent"?"□ 동의   □ 미동의":q.options&&q.options.length?q.options.map(function(o){return (q.type==="rank"?"(   순위) ":"□ ")+o;}).join("    "):q.type==="notice"?"":"________________________________________";
    var text=(i+1)+". "+q.label+(q.required?" *":"")+(q.help?"\n"+q.help:"")+(a?"\n"+a:"");
    if(q.type==="textarea"||text.length>115){flush();body.appendParagraph(text).setSpacingBefore(9).setSpacingAfter(6);}
    else{pair.push(text);if(pair.length===2)flush();}
  });
  flush();doc.saveAndClose();
  var pdf=folder.createFile(file.getAs(MimeType.PDF).setName(name));
  // The editable source is retained with the PDF for recovery and printing.
  var source=getOrCreateFolder_(folder,"출력원본");file.moveTo(source);
  return pdf;
 }catch(e){doc.saveAndClose();throw e;}
}
function nurimCsv_(folder,book){
 ...
 book.getSheets().filter(function(s){return /^v\d+$/.test(s.getName());}).forEach(function(s){
  ...
  s.getRange(2,1,s.getLastRow()-1,9).getDisplayValues().forEach(function(r){lines.push(r.concat(s.getName()));});
 });
...
function authorizeNurimSurveyV78() {
  var doc=DocumentApp.create('누림 설문 출력 권한 점검');
  var id=doc.getId();
  doc.getBody().appendParagraph('출력 권한 확인용 임시 문서');
  doc.saveAndClose();
