(() => {
  ...
  window.nurimParseSchedule = (value) => {
    ...
    const cleaned=text.replace(/^(?:운영|진행|활동|교육|수업|강의|행사|실시|사업|프로그램)\s*(?:기간|일정|일시)\s*[:：]?\s*/,'');
    ...
    const start=iso(date[1],date[2],date[3]);let rest=cleaned.slice(date[0].length),end='';
    if(/^\s*(?:[~～∼–—-]|부터)/.test(rest)){
    ...
    return {start,end,detail:rest.replace(/^[\s/,:：]+/,'').trim()};
  };
