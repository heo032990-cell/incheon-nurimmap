...
runInNewContext(stripTypeScriptTypes(source),{createClient:()=>({from:query}),Deno:{env:{get:()=>''},serve:f=>handler=f},validateAnswers,validateSchema,matchingSignature,Request,Response,crypto,TextEncoder,Intl,Error,console});
async function submit(answers,signature){const r=await handler(new Request('https://test.invalid',{method:'POST',body:JSON.stringify({action:'submit',id:crypto.randomUUID(),token:crypto.randomUUID(),programId:pid,revision:1,basic:{name:'홍길동',phone:'01012345678',birth:'2000-01-01'},answers,extra:{signature,note:'',participantType:'장애 당사자',consentResponses:{consent:'agree'}}})}));return {status:r.status,body:await r.json()};}
